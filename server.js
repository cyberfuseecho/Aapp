...
async function removeBackground(base64Image) {
  ...
  // Resize to 320x320 for model inference only
  const { data: modelData, info: modelInfo } = await sharp(buffer)
    .resize(320, 320, { fit: 'fill' })
    .raw()
    .ensureAlpha()
    .toBuffer({ resolveWithObject: true });

  const { width, height, channels } = modelInfo;
  const floatData = new Float32Array(width * height * 3);

  for (let i = 0; i < width * height; i++) {
    floatData[i] = modelData[i * channels] / 255.0;
    floatData[i + width * height] = modelData[i * channels + 1] / 255.0;
    floatData[i + width * height * 2] = modelData[i * channels + 2] / 255.0;
  }
  ...
  // Build 320x320 grayscale mask with smooth values (0-255)
  const mask320 = Buffer.alloc(320 * 320);
  for (let i = 0; i < 320 * 320; i++) {
    const val = Math.max(0, Math.min(1, maskData[i]));
    mask320[i] = Math.round(val * 255);
  }

  // Resize mask back to original dimensions with smooth interpolation
  const maskResized = await sharp(mask320, {
  ...
  const originalRGBA = await sharp(buffer)
    .resize(origWidth, origHeight, { fit: 'fill' })
    .raw()
    .ensureAlpha()
    .toBuffer();

  // Apply smooth mask to original image
  const outBuffer = Buffer.alloc(origWidth * origHeight * 4);
  for (let i = 0; i < origWidth * origHeight; i++) {
    outBuffer[i * 4] = originalRGBA[i * 4];
    outBuffer[i * 4 + 1] = originalRGBA[i * 4 + 1];
    outBuffer[i * 4 + 2] = originalRGBA[i * 4 + 2];
    outBuffer[i * 4 + 3] = maskResized[i];
  }
